...
export class MediaAudioFiller extends EventEmitter {
    ...
    tick() {
        ...
        if (this.isFirstPacket) {
            ...
            console.log(
                `[MediaAudioFiller] Synced to first packet timestamp: ${this.expectedTimestamp}ms`
            );
            this.emit(
                'data',
                firstPacket.data,
        ...
        if (Math.abs(timeDiff) < this.frameDuration * 3) {
            ...
            const packet = this.buffer.shift();
            console.log(
                `[MediaAudioFiller] Resyncing after large gap: ${timeDiff}ms behind, jumping to ${packet.timestamp}ms`
            );
            dataToEmit = packet.data;
            this.expectedTimestamp = packet.timestamp + this.frameDuration;
            timestampToEmit = packet.timestamp;
            ...
            dataToEmit = this.generateSilentAudioFrame(
                this.sampleRate,
                this.frameDuration
            );
    ...
    logFiller(timestamp, dataSize) {
        ...
        if (!this.lastFillerLog || now - this.lastFillerLog > 1000) {
            console.log(
                `[MediaAudioFiller] 🔊 Filling gap for ${this.userId} at ${timestamp}ms (Buffer: ${this.buffer.length}, Size: ${dataSize})`
            );
    ...
    logReal(timestamp, dataSize) {
        ...
        if (!this.lastRealLog || now - this.lastRealLog > 5000) {
            console.log(
                `[MediaAudioFiller] Emitting real audio for ${this.userId} at ${timestamp}ms (Size: ${dataSize})`
            );
    ...
    stop(endTime) {
        ...
        if (endTime && endTime > this.expectedTimestamp) {
            ...
            for (let i = 0; i < frames; i++) {
                const silentFrame = this.generateSilentAudioFrame(
                    this.sampleRate,
                    this.frameDuration
                );
                this.emit(
