...
export class MediaVideoFiller extends EventEmitter {
    constructor(meetingUuid, streamId, userId, startTime, videoDetails = {}) {
        ...
        try {
            ...
            this.blackFrame = fs.readFileSync(
                path.join(__dirname, 'black_frame.h264')
            );
            this.spsPpsKeyframe = fs.readFileSync(
                path.join(__dirname, 'sps_pps_keyframe.h264')
            );
        } catch (e) {
    ...
    tick() {
        ...
        if (this.isFirstPacket) {
            ...
            console.log(
                `[MediaVideoFiller] Synced to first packet timestamp: ${this.expectedTimestamp}ms`
            );
            this.emit(
                'data',
                firstPacket.data,
        ...
        if (Math.abs(timeDiff) < this.frameDuration * 3) {
            ...
            const packet = this.buffer.shift();
            console.log(
                `[MediaVideoFiller] Resyncing after large gap: ${timeDiff}ms behind, jumping to ${packet.timestamp}ms`
            );
            dataToEmit = packet.data;
            this.expectedTimestamp = packet.timestamp + this.frameDuration;
            timestampToEmit = packet.timestamp;
    ...
    logFiller(timestamp, dataSize) {
        ...
        if (!this.lastFillerLog || now - this.lastFillerLog > 1000) {
            console.log(
                `[MediaVideoFiller] 🎥 Filling gap for ${this.userId} at ${timestamp}ms (Buffer: ${this.buffer.length}, Size: ${dataSize})`
            );
    ...
    logReal(timestamp, dataSize) {
        ...
        if (!this.lastRealLog || now - this.lastRealLog > 5000) {
            console.log(
                `[MediaVideoFiller] Emitting real video for ${this.userId} at ${timestamp}ms (Size: ${dataSize})`
            );
        ...
        if (endTime && endTime > this.expectedTimestamp) {
            ...
            for (let i = 0; i < frames; i++) {
                this.emit(
                    ...
                    this.blackFrame,
                    this.userId,
                    this.expectedTimestamp + i * this.frameDuration,
                    this.meetingUuid,
                    this.streamId
                );
            }
