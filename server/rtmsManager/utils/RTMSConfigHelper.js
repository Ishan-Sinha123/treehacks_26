...
export class RTMSConfigHelper {
    ...
    static normalize(userConfig = {}) {
        ...
        if (normalized.credentials) {
            ...
            if (creds.video && !creds.videoSdk) {
                creds.videoSdk = {
                    ...
                    clientSecret:
                        creds.video.videoClientSecret ||
                        creds.video.clientSecret,
                    secretToken:
                        creds.video.videoSecretToken || creds.video.secretToken,
                };
                delete creds.video;
            }
        ...
        if (
            normalized.mediaTypesFlag != null &&
            normalized.mediaTypes == null
        ) {
    ...
    static getCredentialsForProduct(productType, config) {
        ...
        throw new Error(
            `[RTMSManager] No credentials found for product: ${productType}`
        );
    }
