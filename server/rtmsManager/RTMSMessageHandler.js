...
export class RTMSMessageHandler {
    ...
    constructor(
        ...
        this.emit = (eventName, ...args) => {
            if (eventName === 'error') {
                try {
                    ...
                    const errorText =
                        sourceError &&
                        typeof sourceError.toShortString === 'function'
                            ? sourceError.toShortString()
                            : sourceError?.message ||
                              String(sourceError || eventError);
                    FileLogger.error(
                        `[RTMSManager] Unhandled error event (no error listener): ${errorText}`
                    );
                    return false;
                }
            }
        ...
        if (this.config.useFiller) {
            ...
            if (!audioParams) {
                FileLogger.warn(
                    `[RTMSMessageHandler] Audio filler enabled but mediaParams.audio is missing. Using defaults.`
                );
            }
            if (!videoParams) {
                FileLogger.warn(
                    `[RTMSMessageHandler] Video filler enabled but mediaParams.video is missing. Using defaults.`
                );
            }

            this.audioFiller = new MediaAudioFiller(
                this.rtmsId,
                this.streamId,
                'mixed',
                this.startTime,
            ...
            this.videoFiller = new MediaVideoFiller(
                this.rtmsId,
                this.streamId,
                'mixed',
                this.startTime,
    ...
    connect() {
        FileLogger.log(
            `[Handler:${this.streamId.slice(-8)}] Starting for ${
                this.rtmsType
            } ${this.rtmsId}`
        );
    ...
    stop() {
        FileLogger.log(
            `[Handler:${this.streamId.slice(-8)}] Stopping for ${
                this.rtmsType
            } ${this.rtmsId}`
        );
