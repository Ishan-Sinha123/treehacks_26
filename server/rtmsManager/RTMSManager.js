    ...
    VIDEO_RECORDING: {
        ...
        mediaParams: {
            ...
            video: {
                ...
                dataOpt:
                    RTMS_MEDIA_PARAMS.MEDIA_DATA_OPTION_VIDEO_SINGLE_ACTIVE_STREAM,
...
export class RTMSManager extends EventEmitter {
    ...
    static getActiveConnections() {
        try {
            ...
            FileLogger.warn(
                `[RTMSManager] getActiveConnections failed: ${error.message}`
            );
            return [];
        }
    }
    ...
    static async init(options = {}) {
        if (RTMSManager.#instance) {
            (RTMSManager.#instance.logger || FileLogger).warn(
                '[RTMSManager] Already initialized. Returning existing instance.'
            );
            return RTMSManager.#instance;
        }
    ...
    constructor(options = {}) {
        ...
        this.on('error', (error) => {
            const errorText =
                error && typeof error.toShortString === 'function'
                    ? error.toShortString()
        ...
        this.on('meeting.rtms_started', (payload) => {
            const { meeting_uuid, rtms_stream_id, server_urls, event_ts } =
                payload;
            const creds = RTMSConfigHelper.getCredentialsForProduct(
                'meeting',
                this.config
            );
            this.onStreamStart(
        ...
        this.on('webinar.rtms_started', (payload) => {
            const { webinar_uuid, rtms_stream_id, server_urls, event_ts } =
                payload;
            const creds = RTMSConfigHelper.getCredentialsForProduct(
                'webinar',
                this.config
        ...
        this.on('session.rtms_started', (payload) => {
            const { session_id, rtms_stream_id, server_urls, event_ts } =
                payload;
            const creds = RTMSConfigHelper.getCredentialsForProduct(
                'videoSdk',
                this.config
    ...
    async start() {
        ...
        this._state = 'STARTED';
        this.logger.info(
            `[RTMSManager] Ready - feed RTMS events via emit(event, payload)`
        );
    ...
    stop() {
        return new Promise((resolve) => {
            ...
            for (const handler of handlers) {
                this.logger.info(
                    `[RTMSManager] Stopping ${handler.rtmsType} ${handler.rtmsId}`
                );
                handler.stop();
            }
    ...
    onStreamStart(
        ...
        if (this.connectionManager.has(streamId)) {
            this.logger.warn(
                `[RTMSManager] Duplicate stream ID ${streamId} for ${rtmsType} ${rtmsId}`
            );
            return true;
        }

        this.logger.info(
            `[RTMSManager] Starting ${rtmsType} ${rtmsId} stream ${streamId}`
        );
    ...
    onStreamStop(streamId) {
        ...
        if (handler) {
            this.logger.info(
                `[RTMSManager] Stopping ${handler.rtmsType} ${handler.rtmsId} stream ${streamId}`
            );
            handler.stop();
            ...
            this.logger.warn(
                `[RTMSManager] No handler found for streamId ${streamId}`
            );
        }
    }
    ...
    static archiveStream(streamId, data) {
        ...
        if (instance.streamHistory.size > maxSize) {
            ...
            for (let i = 0; i < entriesToRemove; i++) {
                const oldestStreamId =
                    instance.streamHistoryAccessOrder.shift();
                if (oldestStreamId) {
                    instance.streamHistory.delete(oldestStreamId);
                    instance.logger.log(
                        `[RTMSManager] Evicted stream ${oldestStreamId} from history (LRU)`
                    );
                }
            }
        }
