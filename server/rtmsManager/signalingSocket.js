...
export function connectToSignalingWebSocket(
    ...
    FileLogger.log(
        `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] Connecting...`
    );
    ...
    if (conn.signaling && conn.signaling.socket) {
        ...
        if (
            ...
            FileLogger.warn(
                `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] Duplicate connect attempt blocked (readyState: ${existingState}, state: ${conn.signaling.state}).`
            );
            return;
        }
    }
    ...
    if (conn._signalingHandshakeInFlight) {
        FileLogger.warn(
            `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] Duplicate connect attempt blocked (handshake already in flight).`
        );
        return;
    }
    ...
    if (
        !serverUrls ||
        typeof serverUrls !== 'string' ||
        !serverUrls.startsWith('ws')
    ) {
    ...
    signalingWs.on('open', () => {
        try {
            FileLogger.log(
                `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] Connected, sending handshake`
            );

            if (!conn.shouldReconnect) {
                FileLogger.warn(
                    `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] Aborting - RTMS stopped`
                );
                signalingWs.close();
                return;
            }
    ...
    signalingWs.on('close', (code, reason) => {
        FileLogger.log(
            `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] Closed (code: ${code})`
        );
        conn._signalingHandshakeInFlight = false;
        ...
        if (conn.shouldReconnect) {
            FileLogger.log(
                `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] Reconnecting in 3s...`
            );
