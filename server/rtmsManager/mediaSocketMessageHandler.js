...
export function handleMediaMessage(
    data,
    { conn, mediaWs, signalingSocket, meetingUuid, streamId, mediaType, emit }
) {
    try {
        ...
        switch (msg.msg_type) {
            case 4: // DATA_HAND_SHAKE_RESP
                FileLogger.log(
                    `[Media] [${conn.rtmsType},${meetingUuid},${streamId}] Handshake response (${mediaType}): status=${msg.status_code}`
                );
                ...
                if (msg.status_code === 0) {
                    ...
                    if (
                        ...
                        FileLogger.warn(
                            `[Media] [${conn.rtmsType},${meetingUuid},${streamId}] Disabling reconnect for non-retryable status ${msg.status_code}`
                        );
                    }
