...
export function connectToMediaWebSocket(
    ...
    FileLogger.log(
        `[Media] [${conn.rtmsType},${meetingUuid},${streamId}] Connecting ${mediaType} socket to ${mediaUrl}...`
    );

    const mediaWs = new WebSocket(mediaUrl);

    // Each media type gets its own connection object
    const mediaObj = conn.media[mediaType] || {
        socket: null,
        state: 'idle',
        url: mediaUrl,
    };
    ...
    mediaWs.on('open', () => {
        if (!conn.shouldReconnect) {
            FileLogger.warn(
                `[Media] [${conn.rtmsType},${meetingUuid},${streamId}] Aborting open: RTMS stopped for ${conn.rtmsType} ${meetingUuid}`
            );
            mediaWs.close();
            return;
        }
        ...
        FileLogger.log(
            `[Media] [${conn.rtmsType},${meetingUuid},${streamId}] Generating signature for ${mediaType} handshake`
        );

        const messageToSign = `${clientId},${meetingUuid},${streamId}`;
        FileLogger.log(
            `[Media] [${conn.rtmsType},${meetingUuid},${streamId}] Message to sign: ${messageToSign}`
        );
        ...
        FileLogger.log(
            `[Media] [${
                conn.rtmsType
            },${meetingUuid},${streamId}] ${mediaType} handshake payload: ${JSON.stringify(
                handshakeMsg,
                null,
                2
            )}`
    ...
    mediaWs.on('close', async () => {
        FileLogger.warn(
            `[Media] [${conn.rtmsType},${meetingUuid},${streamId}] ${mediaType} socket closed`
        );
        mediaObj.state = 'closed';

        if (!conn.shouldReconnect) {
            FileLogger.log(
                `[Media] [${conn.rtmsType},${meetingUuid},${streamId}] Not reconnecting — RTMS was stopped.`
            );
            return;
        }
        ...
        if (
            ...
            FileLogger.log(
                `[Media] [${conn.rtmsType},${meetingUuid},${streamId}] Reconnecting in 3s...`
            );
            setTimeout(() => {
            ...
            FileLogger.warn(
                `[Media] [${conn.rtmsType},${meetingUuid},${streamId}] Signaling not ready. Restarting both sockets...`
            );
            conn.connect();
        }
    });
    ...
    mediaWs.on('error', (err) => {
        FileLogger.error(
            `[Media] [${conn.rtmsType},${meetingUuid},${streamId}] ${mediaType} socket error: ${err.message}`
        );
