...
export function handleSignalingMessage(
    ...
    try {
        ...
        FileLogger.warn(
            `[Signaling] [${
                conn.rtmsType
            },${meetingUuid},${streamId}] Invalid JSON message: ${data.toString()}`
        );
        return;
    }
    ...
    switch (msg.msg_type) {
        case 2: // SIGNALING_HAND_SHAKE_RESP
            FileLogger.log(
                `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] Handshake response for ${conn.rtmsType} ${meetingUuid}`
            );
            ...
            if (msg.status_code === 0) {
                ...
                const countryCode =
                    hostname.split('.').slice(-3, -2)[0] || 'unknown';
                FileLogger.log(
                    `[Signaling] [${
                        conn.rtmsType
                    },${meetingUuid},${streamId}] Handshake OK. Media URL: ${mediaUrl} (Server: ${countryCode.toUpperCase()})`
                );
                ...
                const effectiveFlags = RTMSFlagHelper.calculateEffectiveFlags(
                    mediaTypesFlag,
                    msg.media_server?.server_urls
                );

                FileLogger.log(
                    `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] Requested media: ${mediaTypesFlag}, available: ${effectiveFlags}`
                );

                // Check if unified mode is enabled (single socket for all media types)
                const useUnifiedMode =
                    conn.config?.useUnifiedMediaSocket === true;

                if (useUnifiedMode) {
                    // Unified mode: single WebSocket for all media types (better sync)
                    FileLogger.log(
                        `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] Connecting unified media socket`
                    );
                    ...
                    for (const [type, flag] of Object.entries(TYPE_FLAGS)) {
                        if (effectiveFlags & flag) {
                            const typeUrl =
                                msg.media_server?.server_urls?.[type] ||
                                mediaUrl;
                            FileLogger.log(
                                `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] Connecting ${type} media socket`
                            );
                            connectToMediaWebSocket(
                                typeUrl,
                                meetingUuid,
                ...
                FileLogger.log(
                    `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] Subscribing to events`
                );
                ...
                FileLogger.error(
                    `[Signaling] [${
                        conn.rtmsType
                    },${meetingUuid},${streamId}] ${error.toShortString()}`
                );
                ...
                if (
                    ...
                    FileLogger.warn(
                        `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] Disabling reconnect for non-retryable status ${msg.status_code}`
                    );
                }
        ...
        case 6: // Events
            if (msg.event) {
                switch (msg.event.event_type) {
                    case 0: // UNDEFINED
                        FileLogger.log(
                            `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] UNDEFINED event`
                        );
                        break;

                    case 1: // FIRST_PACKET_TIMESTAMP
                        FileLogger.log(
                            `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] FIRST_PACKET_TIMESTAMP: ${msg.event.timestamp}`
                        );
                    ...
                    case 2: // ACTIVE_SPEAKER_CHANGE
                        FileLogger.log(
                            `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] ACTIVE_SPEAKER: ${msg.event.user_name} (ID: ${msg.event.user_id})`
                        );
                        break;
                    ...
                    case 3: // PARTICIPANT_JOIN
                        if (
                            ...
                            Array.isArray(msg.event.participants)
                        ) {
                            msg.event.participants.forEach((p) => {
                                FileLogger.log(
                                    `[Signaling] [${
                                        conn.rtmsType
                                    },${meetingUuid},${streamId}] JOIN: ${
                                        p.user_name || 'Unknown'
                                    }`
                                );
                            });
                    ...
                    case 5: // SHARING_START
                        FileLogger.log(
                            `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] SHARING_START`
                        );
                        break;

                    case 6: // SHARING_STOP
                        FileLogger.log(
                            `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] SHARING_STOP`
                        );
                        break;

                    case 7: // MEDIA_CONNECTION_INTERRUPTED
                        FileLogger.log(
                            `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] MEDIA_CONNECTION_INTERRUPTED`
                        );
                        break;

                    default:
                        FileLogger.log(
                            `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] Unknown event_type: ${msg.event.event_type}`
                        );
                }
            }
        ...
        case 8: // Stream State changed
            FileLogger.log(
                `[Signaling] [${
                    conn.rtmsType
                },${meetingUuid},${streamId}] Stream state: ${getRtmsStreamState(
                    msg.state
                )}, reason: ${getRtmsStopReason(msg.reason)}`
            );

            // Meeting ended (reason: 6, state: 4)
            if (msg.reason === 6 && msg.state === 4) {
                FileLogger.log(
                    `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] Meeting ended, cleaning up`
                );
        ...
        case 9: // Session State Changed
            FileLogger.log(
                `[Signaling] [${
                    conn.rtmsType
                },${meetingUuid},${streamId}] Session state: ${getRtmsSessionState(
                    msg.state
                )}, stop_reason: ${getRtmsStopReason(msg.stop_reason)}`
            );
        ...
        default:
            FileLogger.log(
                `[Signaling] [${conn.rtmsType},${meetingUuid},${streamId}] Unhandled msg_type: ${msg.msg_type}`
            );
            break;
    }
}
