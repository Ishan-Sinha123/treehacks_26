...
export class FrontendManager {
    ...
    setup() {
        if (!this.app) {
            this.logger.warn(
                '[FrontendManager] ⚠️ No Express app provided. Skipping frontend setup.'
            );
            return;
        }
        ...
        if (this.config.serveStaticEnabled === false) {
            this.logger.info(
                '[FrontendManager] ⏩ Static file serving disabled.'
            );
            return;
        }
        ...
        const viewsPath =
            this.config.viewsPath || path.join(__dirname, 'public/views');
        ...
        this.logger.info(
            `[FrontendManager] 📁 Serving static files from: ${publicPath}`
        );
        ...
        this.app.get('/', (req, res) => {
            ...
            if (!isValidWsUrl) {
                ...
                this.logger.info(
                    `[FrontendManager] Auto-generated WebSocket URL: ${wsUrl}`
                );
            } else {
                this.logger.info(
                    `[FrontendManager] Using configured WebSocket URL: ${wsUrl}`
                );
            }
        ...
        this.logger.info(
            `[FrontendManager] 📱 Static views served at http://localhost:${this.config.port}`
        );
    }
}
