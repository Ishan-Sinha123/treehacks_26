...
export class FrontendWssManager {
    ...
    setup() {
        ...
        this.wss.on('connection', (ws) => {
            ...
            this.logger.log(
                '[FrontendWssManager] 🌐 Frontend client connected (unregistered, registering now...)'
            );
            ...
            const registrationTimeout = setTimeout(() => {
                if (!ws.meetingUUID || !ws.userID) {
                    this.logger.log(
                        '[FrontendWssManager] ❌ Registration timeout. Closing connection.'
                    );
                    ws.send(
                        JSON.stringify({
                            type: 'error',
                ...
                try {
                    ...
                    if (data.type === 'register') {
                        ...
                        if (meetingUUID && userID) {
                            ...
                            this.logger.log(
                                `[FrontendWssManager] ✅ Client registered: ${userID} for meeting ${meetingUUID}`
                            );
                        } else {
                            this.logger.log(
                                `[FrontendWssManager] ❌ Registration rejected: Invalid meetingUUID or userID`
                            );
                            ws.send(
                                JSON.stringify({
                                    type: 'error',
            ...
            ws.on('close', () => {
                ...
                const info =
                    ws.userID && ws.meetingUUID
                        ? `: ${ws.userID} from ${ws.meetingUUID}`
                        : '';
                this.logger.log(
                    `[FrontendWssManager] ❌ Frontend client disconnected${info}`
                );
            });
        ...
        this.logger.log(
            `[FrontendWssManager] 🧩 Frontend WSS initialized at ${this.config.frontendWssPath}`
        );
    }
    ...
    broadcastToUser(meetingUUID, userID, message) {
        ...
        for (const client of this.frontendClients) {
            if (
                client.readyState === 1 &&
                client.meetingUUID === meetingUUID &&
    ...
    stop() {
        ...
        for (const client of this.frontendClients) {
            try {
                ...
                this.logger.error(
                    '[FrontendWssManager] Error closing client connection:',
                    err
                );
            }
        }
        ...
        if (this.wss) {
            try {
                this.wss.close(() => {
                    this.logger.log(
                        '[FrontendWssManager] WebSocket server closed'
                    );
                });
                this.wss = null;
            } catch (err) {
                this.logger.error(
                    '[FrontendWssManager] Error closing WebSocket server:',
                    err
                );
            }
        }
        ...
        this.logger.log(
            '[FrontendWssManager] Stopped and cleaned up all connections'
        );
    }
}
